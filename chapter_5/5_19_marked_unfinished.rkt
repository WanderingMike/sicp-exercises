...
(define (make-machine register-names ops controller-text)
  (let ((machine (make-new-machine))) ; creating one empty machine model
    ...
    (let ((assemble-result (assemble controller-text machine))) ; instruction sequence is converted to the machine representation
      ((machine 'install-instruction-sequence) (cdr assemble-result)) ; passing a message to install instruction sequence
      ((machine 'install-breakpoint-controller) (car assemble-result))) 
    machine))
    ...
    (let ((the-ops
      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'get-breakpoint-controller) breakpoint-controller)
           ...
           (cond ((symbol? next-inst)  ; labels
                  ...
                  (receive (cons next-inst insts)
                           (cons (make-label-entry next-inst
                                 ...
                                 labels)))
                 (else (receive (cons (make-instruction next-inst) ; insts
                              insts)
                        labels))))))))
...
(define (make-breakpoint-controller labels)
  (define (set label n)
    (set-instruction-break!
     (list-ref (lookup-label labels label) n)
     `#t (cons label n)))

  (define (cancel label n)
    (set-instruction-break!
     (list-ref (lookup-label labels label) n)
     '#f '()))
